...
import '../utils/markdown_text_extractor.dart';
import 'clipboard_helper.dart';
import 'feedback_dialog.dart';
import 'file_card.dart';
import 'interactive_markdown/interactive_markdown_body.dart';

class ChatMessageItem extends StatefulWidget {
  final Message message;
  final bool isSendingMessage;
  final bool isLastMessage;
  /// Shared mutable cache for decoded image bytes. This map is written to
  /// during rendering when a new image message is encountered, to avoid
  /// repeated base64 decoding. Consumers should pass the same map instance
  /// across all ChatMessageItem widgets in a list.
  final Map<String, Uint8List> imageCache;

  /// Called to submit feedback. Replaces direct threadService dependency.
  final Future<void> Function(String messageId, String feedbackType, String? feedbackMessage)? onFeedbackSubmit;

  /// Called to delete feedback. Replaces direct threadService dependency.
  final Future<void> Function(String messageId)? onFeedbackDelete;

  /// Called after feedback state changes (submit or delete).
  final void Function(String messageId, String? feedbackType, String? feedbackMessage)? onFeedbackChanged;

  /// Called when a prompt button is tapped in markdown.
  final void Function(String prompt)? onSendPrompt;

  /// Builder for the assistant avatar. If null, shows a generic robot icon.
  final Widget Function(BuildContext context, Message message)? assistantAvatarBuilder;

  /// Builder for file cards. If null, uses the package's FileCard with no download.
...
class _ChatMessageItemState extends State<ChatMessageItem> {
    ...
    return RepaintBoundary(
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            if (!isUserMessage) ...[
              _buildAssistantAvatar(context, colorScheme),
  ...
  Widget _buildAssistantAvatar(BuildContext context, ColorScheme colorScheme) {
    if (widget.assistantAvatarBuilder != null) {
      return widget.assistantAvatarBuilder!(context, message);
    }
    // Default fallback: generic robot icon
    return CircleAvatar(
  ...
  void _copyTextToClipboard(BuildContext context) {
    final plainText = MarkdownTextExtractor().extract(message.content);
  ...
  Future<void> _handleFeedbackSubmit(String feedbackType, String? feedbackMessage) async {
    if (widget.onFeedbackSubmit == null) return;

    setState(() => _isSubmitting = true);

    try {
      await widget.onFeedbackSubmit!(message.id, feedbackType, feedbackMessage);
  ...
  Future<void> _handleFeedbackDelete() async {
    if (widget.onFeedbackDelete == null) return;

    setState(() => _isSubmitting = true);

    try {
      await widget.onFeedbackDelete!(message.id);
  ...
  Widget _buildMessageContent(BuildContext context) {
    ...
    if (message.type == 'file_link' && message.content.isNotEmpty) {
      if (widget.fileCardBuilder != null) {
        return widget.fileCardBuilder!(context, message.content);
      }
