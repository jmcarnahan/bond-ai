...
# Validate the compute platform is enabled (EKS is the only platform since the
# App Runner and ECS Express paths were retired)
check "at_least_one_platform" {
  assert {
    condition     = var.enable_eks
    error_message = "enable_eks must be true — EKS is the only compute platform."
  }
}
