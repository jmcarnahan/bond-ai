...
class TestThreads(ABC):
  ...
  def test_grant_thread(self):
    ...
    thread_id = self.threads.create_thread()
    assert thread_id is not None
    threads = self.config.get_metadata().get_current_threads(user_id, count=10)
    assert len(threads) == 1
    assert threads[0]['thread_id'] == thread_id

    threads = self.config.get_metadata().get_current_threads('test_user_2', count=10)
    assert len(threads) == 0  
...
class TestThreadsDB(TestThreads):
  ...
  def teardown_method(self):
    ...
    self.config.get_metadata().close()
